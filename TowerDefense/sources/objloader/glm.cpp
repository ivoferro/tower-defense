...
int glmFindOrAddTexture(GLMmodel* model, char* name, mycallback *call)
{
	...
	model->textures[model->numtextures - 1].id = glmLoadTexture(tgaFilename != NULL ? tgaFilename : filename, GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE, &width, &height);
...
GLMmodel* glmReadOBJ(char* filename, char* tgaFile)
{
	return glmReadOBJ(filename, 0, tgaFile);
}
GLMmodel* glmReadOBJ(char* filename, mycallback *call, char* tgaFile)
{
	if (tgaFile != NULL)
	{
		tgaFilename = (char*)malloc(sizeof(char) * (strlen(tgaFile) + 1));
		strcpy(tgaFilename, tgaFile);
	}
